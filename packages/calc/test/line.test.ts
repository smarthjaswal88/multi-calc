...
describe('computeLine — tax handling', () => {
  it('charges tax on the discounted amount, never the subtotal', () => {
    ...
    expect(result.afterDiscountMinor).toBe(18000);
    expect(result.taxAmountMinor).toBe(900);
  });
  ...
  it('supports a fractional tax rate held in basis points', () => {
    const result = computeLine(line({ unitPriceMinor: 10000, taxPercentBp: 825 }));
    expect(result.taxAmountMinor).toBe(825);
...
describe('rounding', () => {
  ...
  it('rounds below the halfway point down', () => {
    expect(divideRound(4, 3)).toBe(1);
    expect(divideRound(149, 100)).toBe(1);
    expect(divideRound(150, 100)).toBe(2);
  });
  ...
  it('applies a basis-point percentage with one rounding step', () => {
    expect(applyPercentBp(2997, 750)).toBe(225);
    expect(applyPercentBp(2772, 825)).toBe(229);
  });
